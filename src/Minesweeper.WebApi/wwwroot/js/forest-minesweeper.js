...
class ForestMinesweeper {
    ...
    async handleCellClick(e, row, col) {
        ...
        try {
            ...
            if (response.isSuccess) {
                this.currentGame = response.data;
                // Debug: Log board state after reveal
                console.log('🟦 Board state after reveal:', JSON.stringify(this.currentGame.board.cells));
                this.updateCellStates();
                this.updateGameInfo();
                this.checkGameEnd();
            } else {
    ...
    async loadActiveGames() {
        ...
        try {
            // Use the correct endpoint for listing games
            const response = await this.apiCall('/games', 'GET');
            ...
            // Debug: Log all revealed cells after update
            const revealedCells = [];
            for (let r = 0; r < board.rows; r++) {
                for (let c = 0; c < board.columns; c++) {
                    if (board.cells[r][c].isRevealed) {
                        revealedCells.push({ row: r, col: c, mine: board.cells[r][c].hasMine, count: board.cells[r][c].adjacentMineCount });
                    }
                }
            }
